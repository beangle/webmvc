 ...
 */
package org.beangle.webmvc.dev.action.mvc
...
class ConfigAction extends ActionSupport {
  ...
  def beans(): String = {
    ...
    if (Strings.isNotEmpty(parent)) {
      ...
      put("cdi_bundles", ClassLoaders.getResources("META-INF/beangle/cdi.properties"))
    } else {
      put("cdi_bundles", ClassLoaders.getResources("META-INF/beangle/web-cdi.properties"))
    }
