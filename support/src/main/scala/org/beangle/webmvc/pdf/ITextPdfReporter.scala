...
import org.beangle.webmvc.pdf.ITextPdfReporter.Rectangles
...
object ITextPdfReporter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length < 2) {
      println("Usage:ITextPdfReporter /path/to/file.html  /path/to/result.pdf")
    }
    val html = IOs.readString(new FileInputStream(args(0)))
