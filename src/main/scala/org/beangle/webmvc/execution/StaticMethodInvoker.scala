  ...
  def gen(method: Method, mapping: RouteMapping, action: AnyRef): String = {
    ...
    if (method.getParameterTypes.length == 0) {
      ...
      for (argu <- mapping.arguments) {
        ...
        if (arguClassName == "org.beangle.webmvc.context.HeaderArgument" || arguClassName == "org.beangle.webmvc.context.CookieArgument") {
          needRequest = true
          if (parameterTypes(argu_index) != classOf[String]) needConverter = true
        }
        if (arguClassName == "org.beangle.webmvc.context.ParamArgument") {
          needParam = true
          needConverter = true
        }
        argu_index += 1
      ...
      sb ++= "org.beangle.webmvc.context.ActionContext context = org.beangle.webmvc.context.ActionContext$.MODULE$.current();\n"
      if (needConverter)
        sb ++= "org.beangle.commons.collection.MapConverter converter = org.beangle.webmvc.context.Params$.MODULE$.converter();\n"
      if (needRequest)
      ...
      while (pt_index < parameterTypes.length) {
        ...
        if (pt == classOf[HttpServletRequest]) {
          ...
          argument.getClass.getName match {
            case "org.beangle.webmvc.context.ParamArgument" =>
              ...
              sb ++= "}\n"
            case "org.beangle.webmvc.context.CookieArgument" =>
              paramAsString = true
            ...
            case "org.beangle.webmvc.context.HeaderArgument" =>
              paramAsString = true
